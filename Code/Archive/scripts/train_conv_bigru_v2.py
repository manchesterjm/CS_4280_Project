...
# star-level split (random, per-star)
stars = np.unique(tic)
...
for ep in range(1, EPOCHS+1):
    ...
    sched.step()
    ap, roc = eval_metrics(model, val_loader)
    print(f"Epoch {ep:03d} | loss {np.mean(losses):.4f} | val AP {ap:.4f} | val ROC {roc:.4f}")
    if ap>best:
        best=ap; patience=PATIENCE
        torch.save(model.state_dict(), MODEL_DIR/"exo_conv_bigru_v2.pt")
    else:
        patience-=1
        if patience<=0:
            print("Early stopping."); break

state=torch.load(MODEL_DIR/"exo_conv_bigru_v2.pt", map_location=DEVICE); model.load_state_dict(state)
tap, troc = eval_metrics(model, test_loader)
print(f"TEST AP {tap:.4f} | ROC {troc:.4f}")
with open(MODEL_DIR/"metrics_conv_bigru_v2.txt","w") as f:
