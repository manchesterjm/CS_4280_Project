"""
Fixed LSTM Training Script for Exoplanet Detection
Addresses Windows multiprocessing issues and class imbalance

Usage:
    python train_lstm_fixed.py --windows_dir "C:\CS_4280_Project\Code\data\windows_train" --epochs 40 --batch_size 256 --lr 1e-4 --hidden 128 --layers 2 --dropout 0.3 --save_dir "C:\CS_4280_Project\Code\runs\lstm_fixed" --amp_dtype fp16 --pos_weight 3.37 --num_workers 0
"""
...
class LightCurveDataset(Dataset):
    ...
    def __init__(self, X, y):
        ...
        # Handle shape - if (N, seq_len), add feature dimension
...
class ExoLSTM(nn.Module):
    """LSTM model for exoplanet detection"""
...
def main():
    parser = argparse.ArgumentParser(description='Train LSTM for exoplanet detection')
    ...
    print(f"[runtime] num_workers={args.num_workers} (0=Windows safe)")
    ...
    # Create dataloaders (num_workers=0 fixes Windows multiprocessing)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"[epoch {epoch:2d}/{args.epochs}] loss={train_loss:.4f} "
              f"val={val_metrics} dt={dt:.1f}s lr={lr:.3e}")
        ...
        if val_metrics['auc'] > best_auc:
            ...
            print(f"[best] improved to {best_auc:.4f}; saved -> {args.save_dir}/best.pt")
