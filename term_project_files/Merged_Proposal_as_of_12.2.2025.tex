...
\textbf{Target Setting.} Malik et al.'s 0.948 AUC establishes the benchmark my BiLSTM must approach. Current results (AUC 0.893 on TESS Sector 1 test data) fall short of this target, but several factors complicate direct comparison: different datasets (Kepler versus TESS), different class distributions (their balanced sampling versus my 11.7\% positive rate), and different evaluation protocols. Nevertheless, closing the gap to 0.948 AUC remains a concrete goal for future optimization.
...
I trained a 4-layer BiLSTM with K-means clustering ($k$=5) on the TESS Sector 1 ground truth dataset. Table~\ref{tab:bilstm_dataset} summarizes the data: 33,051 windows from 13,541 light curves with 11.9\% positive class ratio. The model (3.9M parameters) uses 256 hidden units per direction, dropout 0.311, and positive class weighting (7.41) to handle imbalance. Hyperparameters were optimized via Optuna over 20 trials.
...
Positive Class & 11.9\% & Hidden Units & 256 \\
Training Set & 26,472 & Parameters & 3.9M \\
Test Set & 6,579 & Dropout & 0.311 \\
Window Length & 2048 & Pos Weight & 7.41 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Baseline to Optimized: Progressive Improvement}

The baseline model achieved AUC 0.6947 on the validation set. To improve performance, I ran Optuna hyperparameter optimization over 20 trials, testing 7 hyperparameters including layer count, batch size, learning rate, and dropout. Table~\ref{tab:optimization_results} shows the improvement: AUC increased from 0.6947 to 0.7572 (+9.0\%) by increasing layers from 3 to 4 and optimizing dropout to 0.311.

\begin{table}[h]
    \centering
    \caption{Baseline vs Optimized Model Performance}
    ...
    \begin{tabular}{@{}lrr@{}}
        ...
        Real Planet Detection & 0/300 (0\%) & \textbf{16/300 (5.3\%)} \\
        \midrule
...
To validate generalization, I tested both models on 300 windows from 100 confirmed exoplanet hosts (TESS and Kepler) never seen during training. The baseline model made zero positive predictions despite reasonable validation AUC, indicating poor calibration. The optimized model detected 16/300 windows as planets, with the top candidate TIC 261337380 scoring 0.667 probability. Early real-world testing on 7 TESS targets correctly ranked the L 98-59 system (TIC 307210830, a confirmed 4-planet system) highest at probability 0.762.
...
To test whether the TESS-trained model generalizes to other missions, I evaluated it on 279 windows from 93 confirmed Kepler exoplanet hosts. Table~\ref{tab:crossmission_results} shows the results. The key challenge: TESS uses 2-minute cadence while Kepler used 30-minute cadence (15$\times$ difference), fundamentally changing transit signal morphology.

\begin{table}[h]
\centering
\caption{Cross-Mission Generalization: TESS vs Kepler}
...
On Kepler data, the model predicted all 279 windows as positive (100\% recall, mean probability 0.978). However, since the Kepler test set contains \textit{only} confirmed planets, I cannot compute precision or AUC. The results are \textbf{inconclusive}: either (1) the model learned generalizable transit physics, or (2) it has a high false positive rate (TESS precision was only 41.8\%). Creating a balanced Kepler test set with negative examples would require downloading non-planet Kepler targets and vetting them carefully---time constraints prevented this. Future work should explore domain adaptation or multi-mission training.
...
\textbf{RNN Findings.} The BiLSTM architecture with K-means clustering achieved [AUC TO BE UPDATED] on the 6,579-window test set. The clustering approach proved effective for handling stellar diversity: by grouping windows with similar statistical properties (variability, noise characteristics), the model learned specialized representations for different stellar types. Hyperparameter optimization via Optuna identified optimal settings of 4 BiLSTM layers, 0.311 dropout, and batch size 64. The 50$\times$ dataset expansion from 655 to 33,051 windows enabled training deeper networks without overfitting.
